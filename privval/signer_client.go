...
func (sc *SignerClient) Ping() error {
	response, err := sc.endpoint.SendRequest(mustWrapMsg(&pvproto1.PingRequest{}))
	if err != nil {
...
func (sc *SignerClient) GetPubKey() (crypto.PubKey, error) {
	response, err := sc.endpoint.SendRequest(mustWrapMsg(&pvproto1.PubKeyRequest{ChainId: sc.chainID}))
	if err != nil {
...
func (sc *SignerClient) SignVote(chainID string, vote *cmtproto.Vote) error {
	response, err := sc.endpoint.SendRequest(mustWrapMsg(&pvproto2.SignVoteRequest{Vote: vote, ChainId: chainID}))
	if err != nil {
		return err
	}
...
func (sc *SignerClient) SignProposal(chainID string, proposal *cmtproto1.Proposal) error {
	response, err := sc.endpoint.SendRequest(mustWrapMsg(
		&pvproto1.SignProposalRequest{Proposal: proposal, ChainId: chainID},
	))
	if err != nil {
		return err
	}
